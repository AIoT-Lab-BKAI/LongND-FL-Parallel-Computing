...
class DDPG_Agent(nn.Module):
    ...
    def ddpg_update(self, gamma=0.99, min_value=-np.inf, max_value=np.inf, soft_tau=2e-2):
        ...
        for i in range(int(len(self.replay_buffer)/self.batch_size)):
            ...
            state = torch.DoubleTensor(state).squeeze()
            next_state = torch.DoubleTensor(next_state).squeeze()
            action = torch.DoubleTensor(action).squeeze()
            reward = torch.DoubleTensor(reward)
            done = torch.DoubleTensor(np.float32(done))
    ...
    def get_action(self, local_losses, local_n_samples, local_num_epochs, done):
        ...
        state = torch.DoubleTensor(state).unsqueeze(0)  # current state
